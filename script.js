...
$(document).ready(function() {
    ...
    $('.lightbox-prev').on('click', function(e) {
        ...
        $('#lightbox-image').attr('src', images[currentImageIndex]);
    });
    ...
    $('#download-all-btn').on('click', function() {
        ...
        btn.find('.download-text').text('다운로드 중...');
        
        // Get all image paths
        const imagePaths = [];
        $('.gallery-item').each(function() {
            imagePaths.push($(this).data('image'));
        });
        ...
        // Load all images and add to ZIP
        Promise.all(imagePaths.map((imagePath, index) => {
            return urlToBlob(imagePath)
                .then(blob => {
                    // Extract filename from path (e.g., "img/01.jpg" -> "01.jpg")
                    ...
                    btn.find('.download-text').text(`다운로드 중... ${progress}%`);
                })
                .catch(error => {
        ...
        .then(() => {
            ...
            zip.generateAsync({ type: 'blob' })
                .then(function(content) {
                    ...
                    link.download = 'house-images.zip';
                    ...
                    btn.find('.download-text').text('전체 이미지 다운로드');
                })
                .catch(error => {
